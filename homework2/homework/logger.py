...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        # We'll use these lists to collect the metrics within the current epoch
        train_accuracies = [] 
        val_accuracies = []
        ...
        for iteration in range(20):
            ...
            train_accuracies.append(dummy_train_accuracy)

            global_step += 1

        # Log average train_accuracy at the end of the epoch
        avg_train_acc = torch.mean(torch.cat(train_accuracies))
        # global_step has already been incremented, so the step corresponds to the end of the epoch's training
        ...
        for _ in range(10):
            ...
            # Save additional metrics to be averaged
            val_accuracies.append(dummy_validation_accuracy)

        # Log average val_accuracy at the end of the epoch
        avg_val_acc = torch.mean(torch.cat(val_accuracies))
        # The step should be the same as the final training step of the epoch
